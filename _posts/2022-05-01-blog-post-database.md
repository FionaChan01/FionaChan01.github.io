...
- **关系模型**
	...
	- 码与模式图<a>https://blog.csdn.net/sumaliqinghua/article/details/85872446#commentBox</a></a>
...
- 数据库结构优化设计
  ...
  - 模式优化
    ...
    - 三范式(3NF)
      ...
      > 相对于BCNF，3NF允许存在主属性对候选码的传递依赖和部分依赖
...
- 物理设计 --> 建立物理存储模型
	...
	- 文件组织
	  ...
	  > 考虑用文件表示逻辑数据模型的不同方式
	  >
	  > 每个文件被分成定长的存储单元--块block，块是存储和传输的基本单位
	...
	- 任务
		...
		> 确定定长还是变长记录/ 确定存放次序的搜索码/ 确定是否建立辅助索引文件/ 确定是否采用多表聚集文件存储/ 确定划分为多少个数据库存储/ 确定存放的物理路径
